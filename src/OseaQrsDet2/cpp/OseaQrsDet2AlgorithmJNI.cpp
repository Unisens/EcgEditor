#include "OseaQrsDet2AlgorithmJNI.h"
#include <stdlib.h>

extern int SAMPLE_RATE;
extern int QRSDet(int ecgSample, int init) ;

JNIEXPORT void JNICALL Java_de_fzi_ekgeditor_algorithm_jni_OseaQrsDet2AlgorithmJNI_initJNI (JNIEnv *env, jobject jobject, jintArray initSampleRates){
	SAMPLE_RATE = (*env).GetIntArrayElements(initSampleRates, 0)[0];
	QRSDet(0, 1);
}

JNIEXPORT jintArray JNICALL Java_de_fzi_ekgeditor_algorithm_jni_OseaQrsDet2AlgorithmJNI_process (JNIEnv *env, jobject jobject, jintArray inValues){
