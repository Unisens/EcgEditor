...
int *DDBuffer, DDPtr ;	/* Buffer holding derivative data. */
int Dly  = 0 ;
...
/*	Initialize all buffers to 0 on the first call.	*/
	...
	if(aPeak && !preBlankCnt){			// If there has been no peak for 200 ms									// save this one and start counting.
		...
		if(--preBlankCnt == 0)  // 200 ms pass it on for evaluation.
			newPeak = tempPeak ;
	}

	else if(aPeak){							// If we were holding a peak, but
		if(aPeak > tempPeak){ 				// this ones bigger, save it and
			tempPeak = aPeak ;				// start counting to 200 ms again.
			preBlankCnt = PRE_BLANK ; 	// MS200
		}
